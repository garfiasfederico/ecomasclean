...
class Reporte_Retiro {

    public function create($retiros_id) {
        
        $ModelRetiro = new Model_Retiro();
        $ModelTurno = new Model_Turno();
        
        $infoRetiro = $ModelRetiro->getInfo($retiros_id);        
        $saldoActual = $ModelTurno->getSaldoFinal($infoRetiro->turnos_id);
...
// set document information
        ...
        $pdf->SetTitle('Retiro: '.$retiros_id);
...
$html = "<center><h3>Retiro realizado con Éxito</h3></center>";
$retiro_f = number_format($infoRetiro->monto,2);
$html .= <<<EOD
        <table border="1" style="width:100%">
                <tr>
                        <th style="width:40%;background-color:gray;color:white">Id retiro:</th>
                        <td style="text-align:left">$infoRetiro->id</td>
                </tr>
                <tr>
                        <th style="width:40%;background-color:gray;color:white">Turno:</th>
                        <td style="text-align:left">$infoRetiro->turnos_id</td>
                </tr>
                <tr>
                        <th style="width:40%;background-color:gray;color:white">Fecha:</th>
                        <td style="text-align:left">$infoRetiro->fecha</td>
                </tr>
                <tr>
                        <th style="width:40%;background-color:gray;color:white">Monto:</th>
                        <td style="text-align:left">$ $retiro_f</td>
                </tr>
                <tr>
                        <th style="width:40%;background-color:gray;color:white">Comentario:</th>
                        <td style="text-align:left">$infoRetiro->motivo</td>
                </tr>
                <tr>
...
// This method has several options, check the source code documentation for more information.
        ob_clean();        
        $pdf->Output('Retiro'.$retiros_id.'.pdf', 'I');        
        //die($html);
